...
Agents, in `.claude/agents/`. One orchestrator and six review focuses. Each
focus is one reviewer with two or three briefs as its evaluation criteria;
the agent file carries the focus and the list, and points at `reviewer.md`
for the method.

| Focus | Criteria (briefs under `principles/`) | Extra output |
|---|---|---|
| `mark-review` | Orchestrator. Fans out the six focuses, collates per `evidence.md`, renders the verdict. | |
...
| `review-adversarial` | `adversarial` | coverage and refuted lists |
...
To run: invoke the `mark-review` agent with the scope (default: `git diff
main...HEAD` plus the working tree). Mechanical gates (`make check`,
`make test-assets`, `make check-visual`) are run by the invoker before the
review, one at a time, never by the reviewers.
