...
2. **The scatter.** Search the diff (this is textual content, grep is
   correct here) for the words that mark a quiet narrowing, in comments,
   doc strings, commit messages, and docs: `for now`, `TODO`, `FIXME`,
   `XXX`, `HACK`, `placeholder`, `[PLACEHOLDER]`, `stub`, `not yet`,
   `temporary`, `interim`, `xfail`, `pending()`, `verify manually`; and in
   comment position only (after `//`, `///`, or `#`): `later`, `skip`,
   `pending`, `B\d+ will`. Bare `will` and `later` in prose are English,
   not scatter; the comment marker is the anchor. Each hit is either a
   declared deferral (cite where it is declared) or a finding. List every
   site.
