...
| Builder | Stepwise construction of a complex object | Struct with chained methods returning `Self`; `build()` returns the product | Room construction (`RoomAssembly` is a plain struct today; a builder would name the steps); level construction stages; the credits roll |
...
6. **Naming as literacy.** When a pattern is present on purpose, its name
   is in the type name or the doc comment (`GamePhase` should say State
   machine; a room builder, when one is built, says Builder in its name). Vocabulary shared with the next
