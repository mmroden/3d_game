...
11. **Serena edits code; the pipeline edits products.** The change was made
    with symbol tools on code files (the hook enforces it); products were
    made by stages. A product edited by hand to look right is the same
    defect as a hand install.
