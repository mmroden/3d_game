...
6. **Rule-of-three residue.** Comments or commit messages saying "will
   ...
   owner decision.
7. **Removed behavior with no counted fate.** Every deleted or replaced
   line enforced something: a guard, a validation, a conversion, an
   example, a rule in a doc. For each, name what it enforced and cite where
   the new code re-establishes it, or the owner decision that dropped it.
   A behavior with neither is lost, not moved: it now lives in zero places,
   the inverse of the split this brief hunts. The 2026-09-15 port of the
   review agents dropped the word "just" from "not just a smell" and the
   comment anchors from a scatter list; both read fine as new text and
   both inverted a rule. Cross-report to `silent_deferrals.md` when the
   loss is an undeclared narrowing of what was asked.
...
survives, then name the invariant it carried and where it lives now (check
7); walk the removed lines of docs and tests the same way. For each added
path, confirm the old one is gone or cite it.
...
three residue: CONCERN, because it is a silent deferral. Removed behavior
with no counted fate: BLOCKER when it was a guard, a validation, or a gate;
CONCERN otherwise.
