...
fn grid_dimensions_match_template_extents() {
    let grid = CellGrid::new(&room_3x3(), &[], [0.0, 0.0, 0.0], 4.0, 5.0);
...
fn small_room_grid_has_one_cell() {
    let grid = CellGrid::new(&small_room(), &[], [0.0, 0.0, 0.0], 4.0, 5.0);
...
fn world_centers_respect_origin_offset() {
    let grid = CellGrid::new(&small_room(), &[], [10.0, 5.0, 20.0], 4.0, 5.0);
...
fn multi_story_interior_cell_is_truly_interior() {
    ...
    let grid = CellGrid::new(&tall_room, &[], [0.0, 0.0, 0.0], 4.0, 5.0);
...
fn multi_story_room_has_correct_cell_count() {
    let grid = CellGrid::new(&multi_story_room(), &[], [0.0, 0.0, 0.0], 4.0, 5.0);
...
fn tall_room_unwired_y1_connector_is_not_gap() {
    ...
    let grid = CellGrid::new(&template, active, [0.0, 0.0, 0.0], 4.0, 5.0);
...
fn single_story_5x5_has_interior_cells() {
    ...
    let grid = CellGrid::new(&room, &[], [0.0, 0.0, 0.0], 4.0, 5.0);
