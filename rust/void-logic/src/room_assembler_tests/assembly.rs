...
fn y_only_sealed_cells_have_floor_ceiling_not_walls() {
    ...
    let has_ceiling = placements.iter().any(|p| {
        is_floor_scene(p.scene)
            && at_center(p)
            && (p.position[1] - (center.world_center[1] + CELL_HEIGHT)).abs() < 0.001
    });
...
fn large_room_sealed_has_4_floors_4_ceilings() {
    ...
    // Position verification: floor and ceiling tiles are all at Y=0 and Y=CELL_HEIGHT.
...
fn sealed_small_room_collision_box_count() {
    let boxes = collision_boxes(&small_room(), &[], [0.0, 0.0, 0.0], 4.0);
...
fn sealed_3x3_collision_covers_all_boundaries() {
    let boxes = collision_boxes(&room_3x3(), &[], [0.0, 0.0, 0.0], 4.0);
    ...
    let ceiling_boxes = boxes.iter().filter(|b| b.position[1] > CELL_HEIGHT).count();
...
fn collision_boxes_form_closed_boundary() {
    ...
    let wall_boxes: Vec<_> = boxes.iter().filter(|b| {
        b.position[1] > 0.0 && b.position[1] < CELL_HEIGHT
    }).collect();
