use crate::systems::credits::CreditAccount;
...
mod tests {
    ...
    fn apply_resets_ephemeral_state() {
        let mut run = RunState::new(42);
        run.current_level = 3;
        run.record_kill(crate::systems::enemy_type::EnemyType::GunDrone);
        run.clear_room(0);
        let save = SaveGame::from_run_state(&run);

        let mut fresh = RunState::new(99);
        fresh.record_kill(crate::systems::enemy_type::EnemyType::Bat);
        fresh.clear_room(1);
        save.apply_to(&mut fresh);
